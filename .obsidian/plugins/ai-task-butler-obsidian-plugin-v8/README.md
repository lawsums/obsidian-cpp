...
- **修改任务**：勾选复选框即可切换完成状态。其他编辑功能集中在右键菜单中：
  ...
  - **编辑任务**（替换原"用 Tasks 插件编辑"）：弹出插件自带的 `TaskEditorModal`，**完全不打开/不切换 leaf**，大 Inbox 也不会卡住。弹窗字段：标题、状态（已完成 toggle）、优先级（未指定 / 最高 / 高 / 中 / 低 / 最低）、计划日期 ⏳、开始日期 🛫、截止日期 📅、重复 🔁（自由输入 `every day` / `every weekday` 等）、标签 #（空格分隔，自动补 `#`，留空即清除所有标签）。保存走 `updateTask` 原子写回原文任务行（仍然优先用 `^task-*` 块 ID 定位），导航栏原地刷新。如果装了 Tasks 插件，你仍然可以在 markdown 视图里手动用 `Tasks: Create or edit task`，那是另一条路径，跟本插件的"跳转"无关。
  - **打开原文**：在主编辑区打开任务所在笔记，并把光标定位到任务行（仍走 `getLeaf("split")` + `setActiveLeaf`）。**这是当前唯一会跳转的菜单项**，跟"编辑任务"职责完全分开。
