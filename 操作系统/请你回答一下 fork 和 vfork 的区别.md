...
2. fork（）的父子进程的执行次序不确定；vfork（）保证子进程先运行，在调用exec或exit之前与父进程数据是共享的，在它调用exec 或exit之后父进程才可能被调度运行。
...
不知道，我只知道
fork的作用是调用子进程，可以用于实现多进程关系
fork有一个返回值，会指向子进程的进程号
而子进程需要父进程的进程号的话，可以将父进程的进程号作为fork()参数调用
